...
if ($entradaOK) {
    try {
        ...
        header('Location: ProgramaEN.php');
        exit();
    } catch (PDOException $exception) {
        // Si aparecen errores, se muestra por pantalla el error
        echo('<div class="ejercicio"><span class="error">❌ Ha fallado la conexion: ' . $exception->getMessage() . '</span></div>');
    ...
    <html lang="en">
        ...
        <body>
            <header>
                <h1>Login Logoff</h1>
                <p id="pTitulo">Log in into your account</p>
            </header>
            <main  class="main2">
                ...
                <div class="ejercicioSesion">
                    ...
                    <form name= "loginlogoff" action="<?php echo $_SERVER['PHP_SELF']; ?>" id="form" method="post">
                        <table class="inicioSesion">
                            <tr>
                                <td><input type="text" name="user" id="user" placeholder="User" value="<?php echo(isset($_REQUEST['user']) ? $_REQUEST['user'] : '') ?>"></td>
                                <td class="error"><?php // Se muestra el mensaje de error 
                                    echo($aErrores['user']);?>
                                </td>
                            </tr>  
                            <tr>
                                <td><input type="password" name="pass" id="pass" placeholder="Password" value="<?php echo(isset($_REQUEST['pass']) ? $_REQUEST['pass'] : '') ?>"></td>
                                <td class="error"><?php // Se muestra el mensaje de error 
                                    echo($aErrores['pass']);?>
                                </td>
                            </tr>
                            <tr>
                                <td><input type="submit" form="form" value="Log in" name="enviar" class="botonEnviar"></td>
                            </tr>
                            <tr>
                                <td><input type="submit" form="form" value="Cancel" name="cancelar" class="botonEnviar"></td>
                            </tr>
                        </table>
                    </form>
                </div> 
